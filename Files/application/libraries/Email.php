...
class Email {
    ...
    public function createEmail($template, $data, $language = '') {
        ...
        if($body_text == null) {
            $this->CI->logging->log('Email template not found: ' . $template);
     ...
     * @return string
     */
    private function genFileList($upload_id, $raw_list = false) {
    ...
    public function getPlaceholders($upload_id, $template, $extra_data = array()) {
        ...
        if($upload !== false) {
            ...
            $expire_time = ($upload['time_expire'] - $upload['time']);
            ...
            switch($template) {
                case 'receiver':
                    $data = array(
                        ...
                        'expire_time'   => ($expire_time <= 0 ? lang('never') : $this->secondsToReadable($expire_time)),
    ...
    public function testConnect() {
        try {
            $connected = $this->_smtp_connect();
        } catch (Exception $e) {
            return $e;
        }

        if ($connected) {
            $authenticated = $this->_smtp_authenticate();

            if ($authenticated) {
                return true;
            } else {
                return $this->print_debugger();  // Display detailed error information
            }
        } else {
            return $this->print_debugger();  // Display detailed error information
        }
