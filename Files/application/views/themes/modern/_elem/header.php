...
<nav class="navbar is-info mobile-nav" role="navigation" aria-label="main navigation">
    <div class="navbar-brand">
        <a class="navbar-item" href="<?php echo $settings['site_url'] ?>">
            <img src="<?php echo $settings['mobile_logo_path']; ?>" height="28">
        </a>
        ...
        <div class="navbar-start">
            ...
            if(is_array($extra_pages) && !empty($extra_pages)) {
                foreach ($extra_pages AS $key => $tab) {
                    echo '<a class="navbar-item" '.($tab['type'] == 'link' ? 'href="'.(strpos($tab['content'], 'http') === false ? base_url($tab['content']) : $tab['content']).'" target="_blank"' : '') . ' data-target="tab-' . ($tab['type'] == 'terms_page' ? 'terms' : $key) . '">' . $tab['title'] . '</a>';
                }
            }
            ?>
...
<div class="tabs is-toggle is-small is-right" id="page-tabs">
    <ul>
        ...
        if(is_array($extra_pages) && !empty($extra_pages)) {
            foreach ($extra_pages AS $key => $tab) {
                echo '<li><a '.($tab['type'] == 'link' ? 'href="'.(strpos($tab['content'], 'http') === false ? base_url($tab['content']) : $tab['content']).'" target="_blank"' : '') . ' data-target="tab-' . ($tab['type'] == 'terms_page' ? 'terms' : $key) . '"><span>' . $tab['title'] . '</span></a></li>';
            }
        }
                ...
                <div class="file">
                    <a href="<?php echo $settings['site_url'].'handler/file?file_id='.$file['id'].'&file_secret='.$file['secret_code'] ?>&download=true" class="download" download>
                        ...
                        if($file['thumb'] == 1) {
                            ...
                            if (in_array(pathinfo($file['file'], PATHINFO_EXTENSION), ['mp4', 'webm', 'ogg', 'mkv', 'mov'])):
                            ?>
                                <i class="lni lni-play" style="font-size: 50px;" onclick="event.preventDefault();openVideoModal('<?php echo $settings['site_url'].'handler/file?file_id='.$file['id'].'&file_secret='.$file['secret_code'] ?>')"></i>
                            <?php else: ?>
                            <lord-icon
                        ...
                        <div class="file-details">
                            ...
                            <span class="play" onclick="event.preventDefault();openVideoModal('<?php echo $settings['site_url'].'handler/file?file_id='.$file['id'].'&file_secret='.$file['secret_code'] ?>')">
