...
class UpdateLib
{
    ...
    public function run() {
        ...
        {
            $this->runSQL($update_dir . '/update_2.5.0.sql');
        }
        if($current_version < '2.5.2')
        {
            $this->runSQL($update_dir . '/update_2.5.2.sql');
        }
        if($current_version < '2.5.3')
        {
            $this->runSQL($update_dir . '/update_2.5.3.sql');
        }
        if($current_version < '2.5.4')
        {
            $this->runSQL($update_dir . '/update_2.5.4.sql');
        }
        if($current_version < '2.5.6')
        {
            $this->runSQL($update_dir . '/update_2.5.6.sql');
        }
        if($current_version < '2.5.7')
        {
            $this->runSQL($update_dir . '/update_2.5.7.sql');
        }
        if($current_version < '2.5.9')
        {
            $this->runSQL($update_dir . '/update_2.5.9.sql');
        }
        if($current_version < '2.6.0')
        {
            $this->runSQL($update_dir . '/update_2.6.0.sql');
        }
        if($current_version < '2.6.4')
        {
            $this->runSQL($update_dir . '/update_2.6.4.sql');
        }
        if($current_version < '2.6.5')
        {
            $this->runSQL($update_dir . '/update_2.6.5.sql');
        }
        if($current_version < '2.6.6')
        {
            $this->runSQL($update_dir . '/update_2.6.6.sql');
        }
        if($current_version < '2.6.9')
        {
            $this->runSQL($update_dir . '/update_2.6.9.sql');
        }
